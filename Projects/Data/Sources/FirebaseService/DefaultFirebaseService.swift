...
public extension DefaultFirebaseService {
    public func observer(collection: FireStoreCollection, document: String) -> Observable<FirebaseData> {
    ...
    public func observeLocation(document: String) -> Observable<CLLocation?> {
        return observer(collection: .Locations, document: document)
            .map { data in
                guard let geopoint = data["location"] as? GeoPoint else { return nil }
    ...
    public func setAccessLevel(_ level: AccessLevel) -> Single<Void> {
...
public extension DefaultFirebaseService {
    public func updateLocation(document: String, location: CLLocation) -> Single<Void> {
        let geopoint = GeoPoint(latitude: location.coordinate.latitude,
                                longitude: location.coordinate.longitude)
        return updateDocument(collection: .Locations, document: document, values: ["location": geopoint])
    }
}
        ...
        return Single.create { single in
            ...
            firebaseReference.putData(data, metadata: metaData) { _, error in
                ...
                firebaseReference.downloadURL { url, _ in
                    guard let url = url else { single(.failure(FireStoreError.unknown)); return }
...
extension DefaultFirebaseService {
    ...
    public func getDocFromSubCollection(firstCollection: FireStoreCollection, subCollection: FireStoreCollection, document: String) -> Single<[FirebaseData]> {
        return Single.create { [weak self] single in
            guard let self else { return Disposables.create() }
            self.database.collection(firstCollection.rawValue).document(document).collection(subCollection.rawValue).getDocuments { snapshot, err in
                if let err {
                    single(.failure(err))
                } else {
                    guard let snapshot = snapshot else { single(.failure(FireStoreError.unknown)); return }
                    let data = snapshot.documents.map { $0.data() }
                    single(.success(data))
                }
            }
